...
export type MemoryConsolidationPlan = ConsolidationOutput;
...
/**
 * Origins that must never promote into MEMORY.md on ANY path (append or
 * consolidation). Only EXPLICIT untrusted/system is blocked; candidates without
 * provenance (legacy/unclassified) stay promotable so recall frequency cannot
 * launder externally-derived content, without breaking pre-provenance content.
 */
...
export function isConsolidationCandidateEligible(candidate: PromotionCandidate): boolean {
  ...
  const sessionDerived =
    source === "sessions" ||
    isShortTermSessionCorpusPath(normalizedPath) ||
...
function buildCandidateResultEntry(
  ...
  return `- ${snippet} Source: ${candidateSourceRef(candidate)}`;
}
