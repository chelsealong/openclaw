...
- `--session-key` selects an explicit session key. Agent-prefixed keys must use `agent:<agent-id>:<session-key>`, and `--agent` must match the key's agent id when both are given. Bare non-sentinel keys scope to `--agent` when supplied, or to the sole configured agent otherwise; for example `--agent ops --session-key incident-42` routes to `agent:ops:incident-42`. In an interactive multi-agent terminal, OpenClaw shows an agent picker; non-interactive multi-agent calls must pass `--agent`. The literal keys `global` and `unknown` stay unscoped only when no `--agent` is supplied.
