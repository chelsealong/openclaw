...
| `qa-smoke-ci`                      | Four balanced execution shards of the taxonomy-selected 12-scenario QA smoke set; full taxonomy coverage remains available through explicit QA profiles                                                               | Node-relevant changes                          |
...
Scheduled Matrix checks use the live frontier provider; manual and release Matrix checks use the deterministic mock provider so release transport proof is isolated from model latency and normal provider-plugin startup. Telegram release checks use the same deterministic model boundary. The live transport gateway disables memory search because QA parity covers memory behavior separately; broader provider connectivity is also covered by the separate live model, native provider, and Docker provider suites.

Scheduled, release, and manual Matrix gates use the shared QA Lab suite host and live adapter. They resolve the catalog scenarios eligible for the Matrix channel and requested provider/model first, then split that set across five internal execution shards so eligibility remains independent from CI layout.
